...
const downloadQuestions = async () => {
	...
	if (fs.existsSync(LAST_QUESTION_PAGE_FILE)) {
		const contents: string = fs.readFileSync(LAST_QUESTION_PAGE_FILE).toString();
		lastPage = +contents;
	}
...
const downloadComments = async () => {
	...
	if (fs.existsSync(LAST_COMMENT_PAGE_FILE)) {
		const contents: string = fs.readFileSync(LAST_COMMENT_PAGE_FILE).toString();
		lastPage = +contents;
	}
...
const downloadCommentPage = async (page: number) => {
